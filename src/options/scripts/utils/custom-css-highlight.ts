...
const tokenizeSelector = (value: string): string => {
  const tokens: string[] = [];
  ...
  for (let index = 0; index < value.length; ) {
    ...
    if (char === "@") {
      ...
      tokens.push(wrapToken("custom-css-token-at-rule", value.slice(index, end)));
      index = end;
      continue;
    }

    if (char === "." || char === "#") {
      tokens.push(wrapToken("custom-css-token-selector", char));
      index += 1;
      continue;
    }
    ...
    if (char === ":" && value[index + 1] === ":") {
      tokens.push(wrapToken("custom-css-token-punctuation", "::"));
      index += 2;
      continue;
    }
    ...
    if ("{}(),:>+~=*".includes(char)) {
      tokens.push(
        wrapToken(
          char === "(" || char === ")" || char === "{" || char === "}"
            ? "custom-css-token-bracket"
            : "custom-css-token-punctuation",
          char
        )
      );
      index += 1;
    ...
    if (isIdentifierStart(char) || char === "-") {
      const token = tokenizeIdentifier(value, index, "selector");
      tokens.push(token.html);
      index = token.end;
      continue;
    }
...
const tokenizeValue = (value: string): string => {
  ...
  for (let index = 0; index < value.length; ) {
    ...
    if (value.startsWith("!important", index)) {
      tokens.push(wrapToken("custom-css-token-important", "!important"));
    ...
    if (char === "#") {
      ...
      if (end > index + 1) {
        tokens.push(wrapToken("custom-css-token-number", value.slice(index, end)));
        index = end;
        continue;
      }
    }
    ...
    if (
      isDigit(char) ||
      ((char === "+" || char === "-" || char === ".") && isDigit(value[index + 1]))
    ) {
      const token = tokenizeNumber(value, index);
      tokens.push(token.html);
      index = token.end;
      continue;
    }
    ...
    if (isIdentifierStart(char) || char === "-") {
      const token = tokenizeIdentifier(value, index, "value");
      tokens.push(token.html);
      index = token.end;
      continue;
    }
